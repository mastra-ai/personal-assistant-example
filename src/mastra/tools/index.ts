...
export const blogPostsTool = createTool({
  id: "get-blog-posts",
  description: "Get specified Mastra blog posts from MCP documentation server",
  outputSchema: z.object({
  ...
  execute: async ({ context, mcp }) => {  
    ...
    const posts = await Promise.all(
      blogPosts.map(async (title) => {
        const response = await mcp.docs.search({
          query: title,
          exact: true,
          type: "blog"
        });
        ...
        if (!response?.content) {
          throw new Error(`Blog post not found: ${title}`);
        }
